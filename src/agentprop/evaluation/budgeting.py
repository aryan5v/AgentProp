...
DEFAULT_BUDGET_POLICIES: tuple[AgentBudgetPolicy, ...] = (
    ...
    AgentBudgetPolicy(
        ...
        stop_rule="Verify cancellation, cleanup, and max-concurrency invariants before finalizing.",
    ),
    AgentBudgetPolicy(
        category="build-mode-sensitive",
        planning_steps=2,
        probe_commands=5,
        candidate_sweeps=2,
        verification_runs=3,
        repair_cycles=2,
        max_wall_time_s=1_800,
        stop_rule="Test the verifier's build/run mode; do not trust debug-only success.",
    ),
    AgentBudgetPolicy(
        ...
        repair_cycles=1,
        max_wall_time_s=1_500,
        stop_rule="Numerically verify every explicit domain constraint and threshold.",
    ),
    AgentBudgetPolicy(

...
**Date:** 2026-06-01  
**Tasks:** 10 self-contained coding challenges (see `dev/benchmarks/real_routing_tasks.json`)  
**Harness:** `dev/experiments/run_real_routing_case_study.py`  
...
Pass/fail is determined by executing the LLM-generated code in a subprocess against the exact test assertions in `dev/benchmarks/real_routing_tasks.json`. There is no rubric, no keyword matching, no partial credit — the code either passes or it does not.
...
*Generated by `dev/experiments/run_real_routing_case_study.py`. Raw results in `results.json`. Per-stage outputs in `outputs.jsonl`.*

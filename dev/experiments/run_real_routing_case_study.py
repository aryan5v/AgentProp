...
Run with a real model:
    ...
    PYTHONPATH=src python dev/experiments/run_real_routing_case_study.py \\
        --tasks dev/benchmarks/real_routing_tasks.json

Plumbing self-test without a key (uses reference solutions, NOT a real result):

    PYTHONPATH=src python dev/experiments/run_real_routing_case_study.py --fake
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--tasks",
        type=Path,
        default=Path("dev/benchmarks/real_routing_tasks.json"),
    )
